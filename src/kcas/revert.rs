    ...
    F: FnOnce() -> Result<(), KCasError>,
{
    match revert(
        ...
        Err(revert_error) => match revert_error {
            RevertError::TargetAddressWasNotValidPointer(actual_value) => Err(KCasError::Fatal(
                FatalError::TargetAddressWasNotValidPointer(actual_value),
            )),
        },
    }
}
    ...
    F: FnOnce() -> Result<(), HelpError>,
{
    match revert(
        ...
        Ok(_) => next_function(),
        Err(revert_error) => match revert_error {
            RevertError::TargetAddressWasNotValidPointer(actual_value) => Err(HelpError::Fatal(
                FatalError::TargetAddressWasNotValidPointer(actual_value),
            )),
        },
    }
...
pub(super) fn revert<const NUM_THREADS: usize, const NUM_WORDS: usize>(
    ...
    for word_num in (0..=starting_word_num).rev() {
        ...
        let target_reference: &AtomicUsize = unsafe {
            target_address
                .as_ref()
                .ok_or_else(|| RevertError::TargetAddressWasNotValidPointer(word_num))?
        };
        ...
        _ = target_reference.compare_exchange(
            thread_and_sequence,
            expected_element,
            Ordering::AcqRel,
            Ordering::Acquire,
