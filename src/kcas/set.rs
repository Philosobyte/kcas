...
use crate::kcas::{combine_stage_and_sequence, reset_for_next_operation, HelpError, State};
...
pub(super) fn set_and_transition<const NUM_THREADS: usize, const NUM_WORDS: usize>(
    ...
    match set_result.unwrap_err() {
        ...
        SetError::TargetAddressWasNotValidPointer(word_num) => Err(KCasError::Fatal(
            FatalError::TargetAddressWasNotValidPointer(word_num),
        )),
...
pub(super) fn help_set_and_transition<const NUM_THREADS: usize, const NUM_WORDS: usize>(
    ...
    match set_result.unwrap_err() {
        ...
        SetError::TargetAddressWasNotValidPointer(word_num) => Err(HelpError::Fatal(
            FatalError::TargetAddressWasNotValidPointer(word_num),
        )),
        SetError::ValueWasNotClaimMarker {
            word_num,
...
fn set<const NUM_THREADS: usize, const NUM_WORDS: usize>(
    ...
    for word_num in 0..NUM_WORDS {
        ...
        let target_reference: &AtomicUsize = unsafe {
            target_address
                .as_ref()
                .ok_or_else(|| SetError::TargetAddressWasNotValidPointer(word_num))?
        };

        let desired_element: usize =
            shared_state.desired_elements[thread_index][word_num].load(Ordering::Acquire);
